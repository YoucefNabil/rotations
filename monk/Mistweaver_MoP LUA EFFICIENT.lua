...
local mw_rot = {
	...
	items_intflask = function()
		...
		if player then
			if not player:isCastingAny() and player:ItemCooldown(76085) == 0
	...
	chi_wave = function()
		local player = Object("player")
		if player then
			if not player:isCastingAny() and player:Talent("Chi Wave")
	...
	lifecocoon = function()
		local player = Object("player")
		if player then
			if not player:isCastingAny() and player:SpellCooldown("Life Cocoon")<.3 and _A.enoughmana(116849) then
				--if not player:LostControl() then
				if player:Stance() == 1 then
					local lowest = Object("lowestall")
					if lowest then 
	...
	surgingmist = function()
		local player = Object("player")
		if player then
			if not player:isCastingAny() and player:BuffStack("Vital Mists")>=5
	...
	renewingmist = function()
		local player = Object("player")
		if player then
			if not player:isCastingAny() and player:SpellCooldown("Renewing Mist")<.3 and _A.enoughmana(115151) then
				--if not player:LostControl() then
				if player:Stance() == 1 then
					local lowest = Object("lowestall")
					if lowest then 
	...
	blackout_mm = function()
		local player = Object("player")
		--local enemycombat = Object("enemies")
		if player then
			if not player:isCastingAny() then
				--if not player:LostControl() then
				if player:Stance() == 1 then
