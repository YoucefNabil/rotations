...
local exeOnLoad = function()
	...
	_A.ChimeraCheck = function()
		...
		if not _A.IsSpellKnown(53209) then return false end
		...
		-- Track elapsed time and current focus
		local time_elapsed = 0
		local current_focus = player:focus() - player:spellcost("Chimera Shot")
	...
	_A.CobraCheck = function() -- we only want to cast Cobra Shot if not enough ressources by the time important cds come up (Idk if this is necessary just yet)
		if player:Spellusable("Steady Shot") then
			local ct = player:SpellCasttime("Steady Shot")
			if player:focus()+((player:spellcooldown("Chimera Shot"))*manaregen())<player:spellcost("Chimera Shot") then return true end
			return ct<=player:spellcooldown("Chimera Shot")
		end
	end
	-------------------------------------------------------
	-------------------------------------------------------
...
survival.rot = {
	...
	chimerashot = function()
		if  player:SpellCooldown("Chimera Shot")<cdcd then
			local lowestmelee = _A.totemtar or Object("lowestEnemyInSpellRange(Arcane Shot)")
			if lowestmelee then
				if _A.ChimeraCheck() and player:SpellUsable("Chimera Shot") then
					return lowestmelee:Cast("Chimera Shot")
					elseif _A.CobraCheck() then return lowestmelee:Cast("Steady Shot") 
				end
			end
		end
	end,
	aimedshot_proc = function()
		if player:buff(82926) and player:SpellCooldown("Aimed Shot")<cdcd and not player:IsCurrentSpell(19434) then
			local lowestmelee = _A.totemtar or Object("lowestEnemyInSpellRange(Arcane Shot)")
			if lowestmelee then
	...
	aimedshot = function() -- and player:buff("Thrill of the Hunt") 
